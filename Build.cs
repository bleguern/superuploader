...
namespace SuperUploader
{
	...
	public class Build
	{
		...
		public static void ConvertXlsToCsv(string filename)
		{
		    ...
		    if (values != null)
		    {
		    	if (File.Exists(filename.Replace(".xls", ".csv")))
		    	{
		    		File.Delete(filename.Replace(".xls", ".csv"));
		    	}
		...
		public static string FormatDataToCsv(object str, string type)
		{
			...
			if (str != System.DBNull.Value)
			{
				...
				else if (type.Equals("date"))
				{
					try
					{
						DateTime temp = Convert.ToDateTime(srtValue);
						
						returnValue = temp.Month.ToString().PadLeft(2, '0') + "/" +
									  temp.Day.ToString().PadLeft(2, '0') + "/" +
									  temp.Year.ToString().Substring(2);
		...
		public static string FormatDataToCsv3(object str)
		{
			return FormatDataToCsv3(str, false);
		}
		
		public static string FormatDataToCsv3(object str, bool isNumeric)
		{
			string returnValue = "";
			...
			if (File.Exists(filename))
			{
				...
				while ((line = sr.ReadLine()) != null)
				{
					if (i == 0)
					{
						...
						{
							table.Columns.Add(colunmHeaders[j].Trim());
						}
					}
					else
					{
						...
						for (int j = 0; j < rowValues.GetLength(0); j++)
						{
							tmpValue = rowValues[j].Trim();
							tmpValue = tmpValue.ToLower();
			        		tmpValue = tmpValue.Replace(';', ' ');
			        		tmpValue = tmpValue.Replace('"', ' ');
			        		tmpValue = tmpValue.Replace('é', 'e');
			        		tmpValue = tmpValue.Replace('è', 'e');
			        		tmpValue = tmpValue.Replace('ë', 'e');
			        		tmpValue = tmpValue.Replace('ê', 'e');
			        		tmpValue = tmpValue.Replace('à', 'a');
			        		tmpValue = tmpValue.Replace('ä', 'a');
			        		tmpValue = tmpValue.Replace('â', 'a');
			        		tmpValue = tmpValue.Replace('ü', 'u');
			        		tmpValue = tmpValue.Replace('ù', 'u');
			        		tmpValue = tmpValue.Replace('û', 'u');
			        		tmpValue = tmpValue.Replace('ö', 'o');
			        		tmpValue = tmpValue.Replace('ô', 'o');
			        		tmpValue = tmpValue.Replace('°', 'o');
			        		tmpValue = tmpValue.Replace('î', 'i');
			        		tmpValue = tmpValue.Replace('ï', 'i');
			        		tmpValue = tmpValue.Replace('µ', 'u');
			        		tmpValue = tmpValue.ToUpper().Trim();
		...
		{
			return WriteCsvFromDataTable(table, filename, false);
		}
		
		public static bool WriteCsvFromDataTable(System.Data.DataTable table, string filename, bool writeHeader)
		{
			if (table != null)
			{
				if (File.Exists(filename))
				...
				if (writeHeader)
				{
					...
					{
						sw.Write(Build.FormatDataToCsv(table.Columns[i].ColumnName));
				...
				foreach (DataRow row in table.Rows)
				{
					for (int i = 0; i < table.Columns.Count; i++)
					{
						...
						{
							sw.Write(Build.FormatDataToCsv(row[i], "date"));
						}
						else
						{
		...
		public static bool WriteCsvFromDataTable3(System.Data.DataTable table, string filename)
		{
			if (table != null)
			{
				if (File.Exists(filename))
				...
				foreach (DataRow row in table.Rows)
				{
					for (int i = 0; i < table.Columns.Count; i++)
					{
						if (row[i].GetType() == Type.GetType("System.String"))
						{
							sw.Write(Build.FormatDataToCsv3(row[i]));
						}
						else
						{
							sw.Write(Build.FormatDataToCsv3(row[i], true));
						}
			...
			foreach (DataRow rowCost in cost.Rows)
			{
				foreach (DataRow rowItem in item.Rows)
				{
					if (rowItem[0].Equals(rowCost[0]))
					{
						...
						row[3] = "Old";                      // csset
						row[4] = "Material";                 // sptwkfl_element
						row[5] = rowCost[37];                // sptwkfl_cst_tl
						
						// Coûts à 0
						if (!rowCost[37].Equals("0"))
						{
			...
			foreach (DataRow rowCostV9 in costV9.Rows)
			{
				foreach (DataRow rowItem in item.Rows)
				{
					if (rowItem[0].Equals(rowCostV9[1]))
					{
						...
						row[3] = "Old";                 // csset
						row[4] = "Subcontr";                 // sptwkfl_element
						row[5] = rowCostV9[4];               // sptwkfl_cst_tl
						
						// Coûts à 0
						if (!rowCostV9[4].Equals("0"))
						{
							costTable.Rows.Add(row);
						}
						...
						row[2] = rowCostV9[2];               // site
						row[3] = "Old";                 // csset
						row[4] = "Labor";                 // sptwkfl_element
						row[5] = rowCostV9[8];               // sptwkfl_cst_tl
						
						// Coûts à 0
						if (!rowCostV9[8].Equals("0"))
						{
							costTable.Rows.Add(row);
						}
						...
						row[2] = rowCostV9[2];               // site
						row[3] = "Old";                 // csset
						row[4] = "Burden";                 // sptwkfl_element
						row[5] = rowCostV9[10];              // sptwkfl_cst_tl
						
						// Coûts à 0
						if (!rowCostV9[10].Equals("0"))
						{
							costTable.Rows.Add(row);
						}
						...
						row[2] = rowCostV9[2];               // site
						row[3] = "Old";                 // csset
						row[4] = "Overhead";                 // sptwkfl_element
						row[5] = rowCostV9[12];              // sptwkfl_cst_tl
						
						// Coûts à 0
						if (!rowCostV9[12].Equals("0"))
						{
							costTable.Rows.Add(row);
						}
		...
		public static System.Data.DataTable Write112_Items(System.Data.DataTable itemCommentFC, System.Data.DataTable itemCommentFB, System.Data.DataTable itemCommentWA)
		{
			System.Data.DataTable itemComment = new System.Data.DataTable();
			DataRow row = null;
			
			#region Format du fichier des commentaires
			// Commentaires
			itemComment.Columns.Add("cd_ref", Type.GetType("System.String"));
			itemComment.Columns.Add("cd_type", Type.GetType("System.String"));
			itemComment.Columns.Add("cd_lang", Type.GetType("System.String"));
			itemComment.Columns.Add("cd_seq", Type.GetType("System.Int32"));
			
			for (int i = 0; i < 15; i++)
			{
				itemComment.Columns.Add("cd_cmmt[" + i + "]", Type.GetType("System.String"));
			}
			#endregion
			
			#region Chargement des commentaires FC
			foreach (DataRow rowItemCommentFC in itemCommentFC.Rows)
			{
				// Commentaires FC : page 1
				row = itemComment.NewRow();
				
				row[0] = rowItemCommentFC[0];        // cd_ref
				row[1] = "FC";                       // cd_type
				row[2] = "FR";                       // cd_lang
				row[3] = "1";                        // cd_seq
				
				row[4] = GetValue(rowItemCommentFC[3], "", 75);
				row[5] = GetValue(rowItemCommentFC[4], "", 75);
				row[6] = GetValue(rowItemCommentFC[5], "", 75);
				row[7] = GetValue(rowItemCommentFC[6], "", 75);
				row[8] = GetValue(rowItemCommentFC[7], "", 75);
				row[9] = GetValue(rowItemCommentFC[8], "", 75);
				row[10] = GetValue(rowItemCommentFC[9], "", 75);
				row[11] = GetValue(rowItemCommentFC[10], "", 75);
				row[12] = GetValue(rowItemCommentFC[11], "", 75);
				row[13] = GetValue(rowItemCommentFC[12], "", 75);
				row[14] = GetValue(rowItemCommentFC[13], "", 75);
				row[15] = GetValue(rowItemCommentFC[14], "", 75);
				row[16] = GetValue(rowItemCommentFC[15], "", 75);
				row[17] = GetValue(rowItemCommentFC[16], "", 75);
				row[18] = "";
				
				itemComment.Rows.Add(row);
				
				// Page 2
				if (!rowItemCommentFC[17].ToString().Equals(""))
				{
					row = itemComment.NewRow();
					...
					row[4] = GetValue(rowItemCommentFC[17], "", 75);
					row[5] = GetValue(rowItemCommentFC[18], "", 75);
					row[6] = GetValue(rowItemCommentFC[19], "", 75);
					row[7] = GetValue(rowItemCommentFC[20], "", 75);
					row[8] = GetValue(rowItemCommentFC[21], "", 75);
					row[9] = GetValue(rowItemCommentFC[22], "", 75);
					row[10] = GetValue(rowItemCommentFC[23], "", 75);
					row[11] = GetValue(rowItemCommentFC[24], "", 75);
					row[12] = GetValue(rowItemCommentFC[25], "", 75);
					row[13] = GetValue(rowItemCommentFC[26], "", 75);
					row[14] = GetValue(rowItemCommentFC[27], "", 75);
					row[15] = GetValue(rowItemCommentFC[28], "", 75);
					row[16] = GetValue(rowItemCommentFC[29], "", 75);
					row[17] = GetValue(rowItemCommentFC[30], "", 75);
					row[18] = "";
				...
				// Page 3
				if (!rowItemCommentFC[31].ToString().Equals(""))
				{
					row = itemComment.NewRow();
					...
					row[4] = GetValue(rowItemCommentFC[31], "", 75);
					row[5] = GetValue(rowItemCommentFC[32], "", 75);
					row[6] = GetValue(rowItemCommentFC[33], "", 75);
					row[7] = GetValue(rowItemCommentFC[34], "", 75);
					row[8] = GetValue(rowItemCommentFC[35], "", 75);
					row[9] = GetValue(rowItemCommentFC[36], "", 75);
					row[10] = GetValue(rowItemCommentFC[37], "", 75);
					row[11] = GetValue(rowItemCommentFC[38], "", 75);
					row[12] = GetValue(rowItemCommentFC[39], "", 75);
					row[13] = GetValue(rowItemCommentFC[40], "", 75);
					row[14] = GetValue(rowItemCommentFC[41], "", 75);
					row[15] = GetValue(rowItemCommentFC[42], "", 75);
					row[16] = GetValue(rowItemCommentFC[43], "", 75);
					row[17] = GetValue(rowItemCommentFC[44], "", 75);
					row[18] = "";
				...
				// Page 4
				if (!rowItemCommentFC[45].ToString().Equals(""))
				{
					row = itemComment.NewRow();
					...
					row[4] = GetValue(rowItemCommentFC[45], "", 75);
					row[5] = GetValue(rowItemCommentFC[46], "", 75);
					row[6] = GetValue(rowItemCommentFC[47], "", 75);
					row[7] = GetValue(rowItemCommentFC[48], "", 75);
					row[8] = GetValue(rowItemCommentFC[49], "", 75);
					row[9] = GetValue(rowItemCommentFC[50], "", 75);
					row[10] = "";
					row[11] = "";
					row[12] = "";
					row[13] = "";
			...
			#region Chargement des commentaires FB
			foreach (DataRow rowItemCommentFB in itemCommentFB.Rows)
			{
				// Commentaires FB : page 1
				row = itemComment.NewRow();
				
				row[0] = rowItemCommentFB[0];        // cd_ref
				row[1] = "FB";                       // cd_type
				row[2] = "FR";                       // cd_lang
				row[3] = "1";                        // cd_seq
				
				row[4] = GetValue(rowItemCommentFB[3], "", 75);
				row[5] = GetValue(rowItemCommentFB[4], "", 75);
				row[6] = GetValue(rowItemCommentFB[5], "", 75);
				row[7] = GetValue(rowItemCommentFB[6], "", 75);
				row[8] = GetValue(rowItemCommentFB[7], "", 75);
				row[9] = GetValue(rowItemCommentFB[8], "", 75);
				row[10] = GetValue(rowItemCommentFB[9], "", 75);
				row[11] = GetValue(rowItemCommentFB[10], "", 75);
				row[12] = "";
				row[13] = "";
				row[14] = "";
				row[15] = "";
			...
			#region Chargement des commentaires WA
			foreach (DataRow rowItemCommentWA in itemCommentWA.Rows)
			{
				// Commentaires WA : page 1
				row = itemComment.NewRow();
				
				row[0] = rowItemCommentWA[0];        // cd_ref
				row[1] = "WA";                       // cd_type
				row[2] = "FR";                       // cd_lang
				row[3] = "1";                        // cd_seq
				
				row[4] = GetValue(rowItemCommentWA[3], "", 75);
				row[5] = GetValue(rowItemCommentWA[4], "", 75);
				row[6] = GetValue(rowItemCommentWA[5], "", 75);
				row[7] = GetValue(rowItemCommentWA[6], "", 75);
				row[8] = GetValue(rowItemCommentWA[7], "", 75);
				row[9] = GetValue(rowItemCommentWA[8], "", 75);
				row[10] = GetValue(rowItemCommentWA[9], "", 75);
				row[11] = GetValue(rowItemCommentWA[10], "", 75);
				row[12] = GetValue(rowItemCommentWA[11], "", 75);
				row[13] = GetValue(rowItemCommentWA[12], "", 75);
				row[14] = GetValue(rowItemCommentWA[13], "", 75);
				row[15] = GetValue(rowItemCommentWA[14], "", 75);
				row[16] = GetValue(rowItemCommentWA[15], "", 75);
				row[17] = GetValue(rowItemCommentWA[16], "", 75);
				row[18] = GetValue(rowItemCommentWA[17], "", 75);
				
				itemComment.Rows.Add(row);
			}
			#endregion
			
			Build.WriteCsvFromDataTable(itemComment, "C:\\TEMP\\1.12_items.csv");
			return itemComment;
		}
		
		public static System.Data.DataTable Write13_Items(System.Data.DataTable itemLogistics, System.Data.DataTable itemV9Logistics, System.Data.DataTable item)
		{
			System.Data.DataTable itemLogisticsTable = new System.Data.DataTable();
			DataRow row = null;
			
			#region Format du fichier des données logistiques
			// Données logistiques
			itemLogisticsTable.Columns.Add("pt_part", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("uc_part", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("uv_lenght", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("uv_width", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("uv_height", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("uv_ean", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("sp_part", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("sp_ean", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("sp_conv", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("pc_part", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("pc_ean", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("pc_conv", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("pl_part", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("pl_height", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("pl_ean", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("pl_conv", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("pl_layer", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("pl_type", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("assort", Type.GetType("System.String"));
			itemLogisticsTable.Columns.Add("st_coeff", Type.GetType("System.String"));
			#endregion		
			
			#region Chargement des données logistiques
			foreach (DataRow rowItemLogistics in itemLogistics.Rows)
			{
				foreach (DataRow rowItem in item.Rows)
				{
					if (rowItem[0].Equals(rowItemLogistics[0]))
					{
						// Données logistiques
						row = itemLogisticsTable.NewRow();
						
						row[0] = rowItemLogistics[0];              // pt_part : OK
						
						if (Build.isInTable(rowItemLogistics[1], item, 0))
						{
							row[1] = rowItemLogistics[1];          // uc_part : OK
						}
						else
						{
							row[1] = "";
						...
						if (row[1].ToString().StartsWith("TI") || 
						    row[1].ToString().StartsWith("TP") ||
						    row[1].ToString().StartsWith("MC") || 
						    row[1].ToString().StartsWith("MR"))
						{
							row[1] = row[1].ToString().Replace("TP", "TR-").Replace("TI", "TR-").Replace("MC", "LM-").Replace("MR", "LM-");
						}
						
						row[2] = rowItemLogistics[10];         // uv_lenght : OK
						row[3] = rowItemLogistics[11];         // uv_width : OK
						row[4] = rowItemLogistics[12];         // uv_height : OK
						row[5] = rowItemLogistics[13];         // uv_ean : OK
						
						if (Build.isInTable(rowItemLogistics[18], item, 0))
						{
							row[6] = rowItemLogistics[18];         // sp_part : OK
						}
						else
						{
							row[6] = "";
						...
						if (row[6].ToString().StartsWith("TI") || 
						    row[6].ToString().StartsWith("TP") || 
						    row[6].ToString().StartsWith("MC") || 
						    row[6].ToString().StartsWith("MR"))
						{
							row[6] = row[6].ToString().Replace("TP", "TR-").Replace("TI", "TR-").Replace("MC", "LM-").Replace("MR", "LM-");
						}
						
						if (!row[6].ToString().Equals(""))
						{
							row[7] = rowItemLogistics[22];         // sp_ean : OK
							row[8] = rowItemLogistics[23];         // sp_conv : OK
						}
						else
						{
							row[7] = "";         // sp_ean : OK
						...
						if (Build.isInTable(rowItemLogistics[27], item, 0))
						{
							row[9] = rowItemLogistics[27];         // pc_part : OK
						}
						else
						{
							row[9] = "";
						...
						if (row[9].ToString().StartsWith("TI") || 
						    row[9].ToString().StartsWith("TP") || 
						    row[9].ToString().StartsWith("MC") || 
						    row[9].ToString().StartsWith("MR"))
						{
							row[9] = row[9].ToString().Replace("TP", "TR-").Replace("TI", "TR-").Replace("MC", "LM-").Replace("MR", "LM-");
						}
						
						if (!row[9].ToString().Equals(""))
						{
							row[10] = rowItemLogistics[31];        // pc_ean : OK
							row[11] = rowItemLogistics[32];        // pc_conv : OK
						}
						else
						{
							row[10] = "";        // pc_ean : OK
						...
						if (!rowItemLogistics[36].ToString().Equals(""))
						{
							row[12] = rowItemLogistics[36];        // pl_part : OK
							row[13] = rowItemLogistics[39];        // pl_height : OK
							row[14] = rowItemLogistics[40];        // pl_ean : OK
							row[15] = rowItemLogistics[41];        // pl_conv : OK
							row[16] = rowItemLogistics[42];        // pl_layer : OK
							row[17] = rowItemLogistics[46];        // pl_type : OK
						}
						else
						{
							row[12] = "";        // pl_part : OK
						...
						row[18] = "";                          // assort : OK
						row[19] = rowItemLogistics[47];        // st_coeff : OK
						
						
						if (!row[2].ToString().Equals("0"))
						{
							Build.AddRow(row, ref itemLogisticsTable, 0);
						}
			...
			#region Chargement des données logistiques V9
			foreach (DataRow rowItemV9Logistics in itemV9Logistics.Rows)
			{
				foreach (DataRow rowItem in item.Rows)
				{
					if (rowItem[0].Equals(rowItemV9Logistics[0]))
					{
						// Données logistiques
						row = itemLogisticsTable.NewRow();
						
						row[0] = rowItemV9Logistics[0];        // pt_part : OK
						row[1] = "";                           // uc_part : OK
						row[2] = rowItemV9Logistics[1];        // uv_lenght : OK
						row[3] = rowItemV9Logistics[2];        // uv_width : OK
						row[4] = rowItemV9Logistics[3];        // uv_height : OK
						row[5] = rowItemV9Logistics[6];        // uv_ean : OK
						row[6] = "";                           // sp_part : OK
						row[7] = "";                           // sp_ean : OK
						row[8] = "";                           // sp_conv : OK
						row[9] = "";                           // pc_part : OK
						...
						row[16] = "";                          // pl_layer : OK
						row[17] = "";                          // pl_type : OK
						row[18] = rowItemV9Logistics[7];       // assort : OK
						row[19] = rowItemV9Logistics[8];       // st_coeff : OK
			...
			Build.WriteCsvFromDataTable3(itemLogisticsTable, "C:\\TEMP\\1.3_items.csv");
			return itemLogisticsTable;
		}
		
		public static bool isInTable(object search, System.Data.DataTable table, int position)
		{
			bool founded = false;
			
			foreach (DataRow row in table.Rows)
			{
				if (row[position].Equals(search))
				{
					founded = true;
					break;
				}
		...
		public static System.Data.DataTable WriteAnalysisCode_Items(System.Data.DataTable itemProduct, System.Data.DataTable itemBrand, System.Data.DataTable item)
		{
			...
			foreach (DataRow rowItemProduct in itemProduct.Rows)
			{
				...
				Build.AddAnalysisCode("Item", level1, rowItemProduct[1].ToString(), ref itemAnalysisCodeTable);
				Build.AddAnalysisCode("Item", level2, rowItemProduct[3].ToString(), ref itemAnalysisCodeTable);
				Build.AddAnalysisCode("Item", level3, rowItemProduct[5].ToString(), ref itemAnalysisCodeTable);
				Build.AddAnalysisCode("Item", level4, rowItemProduct[7].ToString(), ref itemAnalysisCodeTable);
				Build.AddAnalysisCode("Item", level5, rowItemProduct[9].ToString(), ref itemAnalysisCodeTable);
			
				Build.AddAnalysisCodeLink("Item", level1, level2, ref itemAnalysisCodeLinkTable);
				Build.AddAnalysisCodeLink("Item", level2, level3, ref itemAnalysisCodeLinkTable);
				Build.AddAnalysisCodeLink("Item", level3, level4, ref itemAnalysisCodeLinkTable);
				Build.AddAnalysisCodeLink("Item", level4, level5, ref itemAnalysisCodeLinkTable);
			...
			foreach (DataRow rowItemBrand in itemBrand.Rows)
			{
				...
				Build.AddAnalysisCode("Item", level1, rowItemBrand[1].ToString(), ref itemAnalysisCodeTable);
				Build.AddAnalysisCode("Item", level2, rowItemBrand[3].ToString(), ref itemAnalysisCodeTable);
				Build.AddAnalysisCode("Item", level3, rowItemBrand[5].ToString(), ref itemAnalysisCodeTable);
			...
			foreach (DataRow rowItem in item.Rows)
			{
				...
				Build.AddAnalysisCode("Item", tmpItem, rowItem[2].ToString(), ref itemAnalysisCodeTable);
				Build.AddAnalysisCodeSelection("Item", "Article", tmpItem, ref itemAnalysisCodeSelectionTable);
		...
		public static System.Data.DataTable WriteAnalysisCode_Customers(System.Data.DataTable customerTree)
		{
			System.Data.DataTable customerAnalysisCodeTable = new System.Data.DataTable();
			System.Data.DataTable customerAnalysisCodeSelectionTable = new System.Data.DataTable();
			System.Data.DataTable customerAnalysisCodeLinkTable = new System.Data.DataTable();
			System.Data.DataTable customerAnalysisGeneralParamsTable = new System.Data.DataTable();
			
			#region Format du fichier des codes analyses
			// Codes analyses
			customerAnalysisCodeTable.Columns.Add("antype", Type.GetType("System.String"));
			customerAnalysisCodeTable.Columns.Add("an_code", Type.GetType("System.String"));
			customerAnalysisCodeTable.Columns.Add("an_desc", Type.GetType("System.String"));
			customerAnalysisCodeTable.Columns.Add("an_active", Type.GetType("System.String"));
			customerAnalysisCodeTable.Columns.Add("cmmt_yn", Type.GetType("System.String"));
			#endregion
			
			#region Format du fichier des sélections codes analyse
			// Sélections codes analyse
			customerAnalysisCodeSelectionTable.Columns.Add("antype", Type.GetType("System.String"));
			customerAnalysisCodeSelectionTable.Columns.Add("ans_code", Type.GetType("System.String"));
			customerAnalysisCodeSelectionTable.Columns.Add("cond_field", Type.GetType("System.String"));
			customerAnalysisCodeSelectionTable.Columns.Add("ans_mask", Type.GetType("System.String"));
			customerAnalysisCodeSelectionTable.Columns.Add("ans_sel_low", Type.GetType("System.String"));
			customerAnalysisCodeSelectionTable.Columns.Add("sel_high", Type.GetType("System.String"));
			#endregion	
			
			#region Format du fichier des liens codes analyse
			// Liens codes analyse
			customerAnalysisCodeLinkTable.Columns.Add("antype", Type.GetType("System.String"));
			customerAnalysisCodeLinkTable.Columns.Add("anl_code", Type.GetType("System.String"));
			customerAnalysisCodeLinkTable.Columns.Add("link_ancode", Type.GetType("System.String"));
			customerAnalysisCodeLinkTable.Columns.Add("anl_desc", Type.GetType("System.String"));
			#endregion
			
			#region Format du fichier des paramètres généraux
			// Paramètres généraux
			customerAnalysisGeneralParamsTable.Columns.Add("code_fldname", Type.GetType("System.String"));
			customerAnalysisGeneralParamsTable.Columns.Add("code_value", Type.GetType("System.String"));
			customerAnalysisGeneralParamsTable.Columns.Add("code_cmmt", Type.GetType("System.String"));
			#endregion
			
			#region Chargement des codes analyses client
			foreach (DataRow rowCustomerTree in customerTree.Rows)
			{
				string level1 = rowCustomerTree[0].ToString();
				string level2 = rowCustomerTree[2].ToString();
				string level3 = rowCustomerTree[4].ToString();
				string level4 = rowCustomerTree[6].ToString();
				string level5 = rowCustomerTree[8].ToString();
				
				Build.AddAnalysisCode("Customer", level1, rowCustomerTree[1].ToString(), ref customerAnalysisCodeTable);
				Build.AddAnalysisCode("Customer", level2, rowCustomerTree[3].ToString(), ref customerAnalysisCodeTable);
				Build.AddAnalysisCode("Customer", level3, rowCustomerTree[5].ToString(), ref customerAnalysisCodeTable);
				Build.AddAnalysisCode("Customer", level4, rowCustomerTree[7].ToString(), ref customerAnalysisCodeTable);
				Build.AddAnalysisCode("Customer", level5, rowCustomerTree[9].ToString(), ref customerAnalysisCodeTable);
			
				Build.AddAnalysisCodeSelection("Customer", "User Field2", level5, ref customerAnalysisCodeSelectionTable);
				
				Build.AddAnalysisCodeLink("Customer", level1, level2, ref customerAnalysisCodeLinkTable);
				Build.AddAnalysisCodeLink("Customer", level2, level3, ref customerAnalysisCodeLinkTable);
				Build.AddAnalysisCodeLink("Customer", level3, level4, ref customerAnalysisCodeLinkTable);
				Build.AddAnalysisCodeLink("Customer", level4, level5, ref customerAnalysisCodeLinkTable);
				
				Build.AddGeneral("xxcm_structure", level5, rowCustomerTree[9], ref customerAnalysisGeneralParamsTable);
			}
			#endregion
			
			Build.WriteCsvFromDataTable(customerAnalysisCodeTable, "C:\\TEMP\\1.8.1_customers.csv");
			Build.WriteCsvFromDataTable(customerAnalysisCodeSelectionTable, "C:\\TEMP\\1.8.4_customers.csv");
			Build.WriteCsvFromDataTable(customerAnalysisCodeLinkTable, "C:\\TEMP\\1.8.7_customers.csv");
			Build.WriteCsvFromDataTable(customerAnalysisGeneralParamsTable, "C:\\TEMP\\36.2.13_customer_analysis.csv");
			return customerAnalysisGeneralParamsTable;
		}
		...
		private static void AddAnalysisCode(string type, string code, string description, ref System.Data.DataTable itemAnalysisCodeTable)
		{
			DataRow newRow = null;
			bool exists = false;
		...
		private static void AddAnalysisCodeSelection(string type, string condition, string code, ref System.Data.DataTable analysisCodeSelectionTable)
		{
			DataRow newRow = null;
			bool exists = false;
			...
			if (!code.Equals(""))
			{
				foreach (DataRow row in analysisCodeSelectionTable.Rows)
				{
					if (row[1].ToString().Equals(code))
					{
						exists = true;
				...
				if (!exists)
				{
					newRow = analysisCodeSelectionTable.NewRow();
					
					newRow[0] = type;                  // antype
					newRow[1] = code;                  // ans_code
					newRow[2] = condition;             // cond_field
					newRow[3] = "*";                   // ans_mask
					newRow[4] = code;                  // ans_sel_low
					newRow[5] = code;                  // sel_high
					
					analysisCodeSelectionTable.Rows.Add(newRow);
				}
			}
		}
		...
		/// <param name="strValue"></param>
		/// <param name="generalTable"></param>
		private static void AddAnalysisCodeLink(string type, string parentCode, string childCode, ref System.Data.DataTable analysisCodeLinkTable)
		{
			DataRow newRow = null;
			bool exists = false;
			...
			{
				foreach (DataRow row in analysisCodeLinkTable.Rows)
				{
				...
				if (!exists)
				{
					newRow = analysisCodeLinkTable.NewRow();
					...
					analysisCodeLinkTable.Rows.Add(newRow);
				}
			}
		}
			...
			foreach (DataRow rowBusinessRelation in businessRelation.Rows)
			{
				...
				else
				{
					...
					if (rowBusinessRelation[2].ToString().Equals("0"))
					{
						...
						row[55] = rowBusinessRelation[23];     // "Code comté";
						row[56] = "";                          // "Etat";
					...
					else // Contact
					{
						...
						if (!rowBusinessRelation[40].ToString().Equals(tmpContact))
						{
							businessRelationTable.Rows.Add(row);
							tmpContact = rowBusinessRelation[40].ToString();
						}
		...
		public static System.Data.DataTable Write361331_Customers(System.Data.DataTable businessRelation)
		{
			System.Data.DataTable countyTable = new System.Data.DataTable();
			DataRow row = null;
			
			#region Format du fichier des contés
			// Contes
			countyTable.Columns.Add("tCounty.CountyCode", Type.GetType("System.String"));
			countyTable.Columns.Add("tCounty.CountyDescription", Type.GetType("System.String"));
			countyTable.Columns.Add("tCounty.CountyIsActive", Type.GetType("System.String"));
			#endregion
			
			row = countyTable.NewRow();
			
			#region Intitulés des champs des contés
			row[0] = "County";
			row[1] = "Description";
			row[2] = "Active";
			#endregion
			
			countyTable.Rows.Add(row);
			
			#region Chargement des contés
			foreach (DataRow rowBusinessRelation in businessRelation.Rows)
			{
				row = countyTable.NewRow();
				
				#region Contés
				row[0] = GetValue(rowBusinessRelation[23], 20);      // "County";
				row[1] = GetValue(rowBusinessRelation[23], 20);      // "Description";
				row[2] = "yes";                                      // "Active";
				#endregion
				
				Build.AddRow(row, ref countyTable, 0);		
			}
			#endregion
			
			Build.WriteCsvFromDataTable2(countyTable, "C:\\TEMP\\36.1.3.3.1_customers.txt");
			return countyTable;
		}
		
		public static System.Data.DataTable Write361431_Suppliers(System.Data.DataTable businessRelation, System.Data.DataTable supplierV9, System.Data.DataTable supplierCodeV9QAD2008)
		{
			System.Data.DataTable businessRelationTable = new System.Data.DataTable();
			DataRow row = null;
			...
			foreach (DataRow rowBusinessRelation in businessRelation.Rows)
			{
				...
				{	
					...
					else // Contact
					{
						...
						if (!rowBusinessRelation[30].ToString().Equals(tmpContact))
						{
							businessRelationTable.Rows.Add(row);
							tmpContact = rowBusinessRelation[30].ToString();
						}
			...
			#region Chargement des relations d'affaire V9
			foreach (DataRow rowSupplierV9 in supplierV9.Rows)
			{	
				foreach (DataRow rowSupplierCodeV9QAD2008 in supplierCodeV9QAD2008.Rows)
				{
					if (rowSupplierV9[0].ToString().Trim().ToLower().Equals(rowSupplierCodeV9QAD2008[0].ToString().ToLower()))
					{
						row = businessRelationTable.NewRow();
				
						#region Données relation d'affaire
						row[0] = "F" + rowSupplierCodeV9QAD2008[1];  // "Code relation d'affaires";
						row[1] = rowSupplierV9[1];             // "Nom de la relation d'affaires";
						row[2] = "";                           // "Deuxième nom";
						row[3] = "";                           // "Troisième nom";
						row[4] = rowSupplierV9[20];            // "Nom de la recherche";
						row[5] = "";                           // "Code intercompagnies";
						row[6] = "yes";                           // "Cpt";
						row[7] = "no";                           // "Intercompagnie";
						row[8] = "no";                           // "Entité interne";
						row[9] = "no";                         // "Compensation DR/CR";
						row[10] = "";                          // "Code AVR";
						row[11] = "";                          // "Code EAN";
						row[12] = "no";                          // "Etat taxes";
						row[13] = "no";                          // "Dernière archive";
						row[14] = "";                          // "Contrôle nom";
						row[15] = "";                          // "Nom de groupe";
						row[16] = "FR";                        // "Lang";
						row[17] = "";                          // "SalesPriceListCode";
						...
						row[21] = "";                          // "Heure modif";
						row[22] = "";                          // "Utilisateur modif";
						row[23] = rowSupplierV9[2];            // "Adresse";
						row[24] = rowSupplierV9[3];            // "Adresse";
						row[25] = rowSupplierV9[4];            // "Adresse";
						row[26] = rowSupplierV9[7];            // "Code postal";
						row[27] = rowSupplierV9[5];            // "Ville";
						row[28] = rowSupplierV9[1];            // "Nom";
						row[29] = "";                          // "Nom de la recherche";
						row[30] = rowSupplierV9[12].ToString().Replace(" ", "");           // "Téléphone";
						row[31] = "";                          // "E-mail";
						row[32] = "";                          // "Internet";
						row[33] = rowSupplierV9[14].ToString().Replace(" ", "");           // "Télécopie";
						row[34] = rowSupplierV9[8];            // "Format";
						row[35] = "no";                        // "Temp";
						row[36] = "FR";                        // "Zone de taxation";
						row[37] = "40";                        // "Classe de taxe";
						row[38] = "";                          // "Emploi taxe";
						row[39] = "";                          // "Adresse postale";
						row[40] = "";                          // "Adresse postale";
						row[41] = "no";                        // "Envoyer les documents";
						...
						row[44] = "";                          // "Ville";
						row[45] = "";                          // "Comté";
						row[46] = "no";                        // "Adresse de taxation";
						row[47] = "yes";                       // "Ville taxe entrante";
						row[48] = "no";                          // "Taxe comprise";
						row[49] = "";                          // "Taxe fédérale";
						row[50] = "";                          // "Taxe locale";
						row[51] = rowSupplierV9[55];           // "Taxes - Divers 1";
						row[52] = "";                          // "Taxes - Divers 2";
						row[53] = "";                          // "Taxes - Divers 3";
						row[54] = "0";                         // "Déclaration fiscale";
						row[55] = "";                          // "Code comté";
						row[56] = "";                          // "Etat";
						row[57] = rowSupplierV9[9];            // "Pays";
						row[58] = "HEADOFFICE";                // "Type";
						row[59] = "FR";                        // "Lang";
						row[60] = "";                          // "Description";
						row[61] = "";                          // "Description";
						row[62] = "";                          // "Description";
						...
						if (rowSupplierV9[11].ToString().Equals(""))
						{
							row = businessRelationTable.NewRow();
					
							#region Données contact (1)
							row[0] = "";                           // "Code relation d'affaires";
							row[1] = "";                           // "Nom de la relation d'affaires";
							row[2] = "";                           // "Deuxième nom";
							row[3] = "";                           // "Troisième nom";
							...
							row[68] = "";                          // "Utilisateur modif";
							row[69] = "";                          // "Fonction";
							row[70] = rowSupplierV9[11];           // "Nom";
							row[71] = "";                          // "Initiales";
							row[72] = "male";                      // "Sexe";
							row[73] = "";                          // "Titre";
							row[74] = rowSupplierV9[12].ToString().Replace(" ", "");           // "Téléphone";
							row[75] = "";                          // "Portable";
							row[76] = "";                          // "E-mail";
							row[77] = rowSupplierV9[14].ToString().Replace(" ", "");           // "Télécopie";
							row[78] = "";                          // "Primaire";
							row[79] = "";                          // "Secondaire";
							row[80] = "FR";                        // "Lang";
							row[81] = "";                          // "Date modif";
						...
						if (!rowSupplierV9[15].ToString().Equals(""))
						{
							row = businessRelationTable.NewRow();
				
							#region Données d'adresse (Stockiste)
							row[0] = "";                           // "Code relation d'affaires";
							row[1] = "";                           // "Nom de la relation d'affaires";
							row[2] = "";                           // "Deuxième nom";
							row[3] = "";                           // "Troisième nom";
							...
							if (rowSupplierCodeV9QAD2008[1].ToString().Contains("TR"))
							{
								row[23] = "2 RUE DE VARENNES";         // "Adresse";
								row[24] = "";                          // "Adresse";
								row[25] = "";                          // "Adresse";
								row[26] = "10140";                     // "Code postal";
								row[27] = "VENDEUVRE SUR BARSE";       // "Ville";
								row[28] = "GAMBA ROTA";                // "Nom";
							}
							else
							{
								row[23] = "ZI DU FELET";               // "Adresse";
								row[24] = "BP 69";                     // "Adresse";
								row[25] = "";                          // "Adresse";
								row[26] = "63307";                     // "Code postal";
								row[27] = "THIERS CEDEX";              // "Ville";
								row[28] = "TRANSPORT COMBRONDE SAS";   // "Nom";
							}
							
							row[29] = "";                          // "Nom de la recherche";
							row[30] = rowSupplierV9[16].ToString().Replace(" ", "");           // "Téléphone";
							row[31] = "";                          // "E-mail";
							row[32] = "";                          // "Internet";
							row[33] = rowSupplierV9[18].ToString().Replace(" ", "");           // "Télécopie";
							row[34] = "1";                         // "Format";
							row[35] = "no";                        // "Temp";
							row[36] = "FR";                        // "Zone de taxation";
							row[37] = "40";                        // "Classe de taxe";
							...
							row[68] = "";                          // "Utilisateur modif";
							row[69] = "";                          // "Fonction";
							row[70] = rowSupplierV9[15];           // "Nom";
							row[71] = "";                          // "Initiales";
							row[72] = "male";                      // "Sexe";
							row[73] = "";                          // "Titre";
							row[74] = rowSupplierV9[16].ToString().Replace(" ", "");  // "Téléphone";
							row[75] = "";                          // "Portable";
							row[76] = "";                          // "E-mail";
							row[77] = rowSupplierV9[18].ToString().Replace(" ", "");           // "Télécopie";
							row[78] = "";                          // "Primaire";
							row[79] = "";                          // "Secondaire";
							row[80] = "FR";                        // "Lang";
							row[81] = "";                          // "Date modif";
		...
		public static System.Data.DataTable Write282011_Suppliers(System.Data.DataTable supplierFinancial, System.Data.DataTable supplierV9, System.Data.DataTable supplierCodeV9QAD2008)
		{
			...
			#region Chargement des fournisseurs finances V9
			foreach (DataRow rowSupplierV9 in supplierV9.Rows)
			{	
				foreach (DataRow rowSupplierCodeV9QAD2008 in supplierCodeV9QAD2008.Rows)
				{
					if (rowSupplierV9[0].ToString().Trim().ToLower().Equals(rowSupplierCodeV9QAD2008[0].ToString().ToLower()))
					{
						...
						row[0] = "yes";
						row[1] = rowSupplierCodeV9QAD2008[1];
						row[2] = "SERVICE";
						row[3] = "NONE";
						row[4] = "no";
						row[5] = "no";
						...
						row[31] = "no";
						row[32] = "";
						row[33] = "F" + rowSupplierCodeV9QAD2008[1].ToString();
						row[34] = "EUR";
						row[35] = "TILO";
						row[36] = "NORMAL";
						row[37] = "ACHATSTIERS";
		...
		public static System.Data.DataTable Write211_Customers(System.Data.DataTable customer)
		{
			...
			foreach (DataRow rowCustomer in customer.Rows)
			{
				...
				if (rowCustomer[26].ToString().ToLower().Equals("no") ||
				   	rowCustomer[26].ToString().ToLower().Equals("n"))
				{
					row[16] = "4";                              // cm_class
				}
				else
				...
				if (row[9].ToString().Equals("TRD") || row[9].ToString().Equals("GMS"))
				{
					row[36] = "yes";
				}
				else
		...
		public static System.Data.DataTable Write231_Suppliers(System.Data.DataTable supplier, System.Data.DataTable supplierV9, System.Data.DataTable supplierCodeV9QAD2008)
		{
			...
			#region Chargement des fournisseurs V9
			foreach (DataRow rowSupplierV9 in supplierV9.Rows)
			{	
				foreach (DataRow rowSupplierCodeV9QAD2008 in supplierCodeV9QAD2008.Rows)
				{
					if (rowSupplierV9[0].ToString().Trim().ToLower().Equals(rowSupplierCodeV9QAD2008[0].ToString().ToLower()))
					{
						...
						#region Données fournisseur
						row[0] = rowSupplierCodeV9QAD2008[1];          // vd_addr
						row[1] = "";                      // vd_shipvia
						row[2] = "";                      // vd_rmks
						row[3] = "";                      // vd_carrier_id
						row[4] = "no";                    // vd_kanban_supplier
		...
		public static System.Data.DataTable Write115_Customers(System.Data.DataTable customerItem)
		{
			...
			customerItemTable.Columns.Add("xxcp_start", Type.GetType("System.DateTime"));
			...
			customerItemTable.Columns.Add("l_start", Type.GetType("System.DateTime"));
			customerItemTable.Columns.Add("l_end", Type.GetType("System.DateTime"));
			...
			foreach (DataRow rowCustomerItem in customerItem.Rows)
			{
				...
				if (!rowCustomerItem[0].ToString().StartsWith("S") && rowCustomerItem[0].ToString().Length == 8)
				{
					row[0] = rowCustomerItem[0].ToString().Substring(0, 7);    // xxcp_cust
				}
				else
				{
				...
				row[2] = rowCustomerItem[3];          // xxcp_start
				row[3] = rowCustomerItem[2];          // xxcp_part
				row[4] = rowCustomerItem[5];          // xxcp_ship_site
				row[5] = rowCustomerItem[6];          // xxcp_ship_whse
				row[6] = System.DBNull.Value;         // l_start
				row[7] = rowCustomerItem[4];          // l_end
		...
		/// <returns></returns>
		public static System.Data.DataTable Write14131_Routing(System.Data.DataTable routing, System.Data.DataTable routingV9, System.Data.DataTable routingComment, System.Data.DataTable routingCommentV9, System.Data.DataTable item)
		{
			...
			{
				routingTable.Columns.Add("cd_seq[" + i + "]", Type.GetType("System.Int32"));
				routingTable.Columns.Add("cd_ref[" + i + "]", Type.GetType("System.String"));
				routingTable.Columns.Add("cd_type[" + i + "]", Type.GetType("System.String"));
				routingTable.Columns.Add("cd_lang[" + i + "]", Type.GetType("System.String"));
				routingTable.Columns.Add("cd_seq_2[" + i + "]", Type.GetType("System.Int32"));
				
				for (int j = 0; j < 15; j++)
				{
					routingTable.Columns.Add("cd_cmmt[" + i + "][" + j + "]", Type.GetType("System.String"));
				}
			}
			#endregion
			...
			foreach (DataRow rowRouting in routing.Rows)
			{
				...
				row[25] = "";                                     // ro_wipmtl_part
				row[26] = System.DBNull.Value;                    // ro_mv_nxt_op
				row[27] = System.DBNull.Value;                    // ro_auto_lbr
				
				foreach (DataRow rowRoutingComment in routingComment.Rows)
				{
					if (rowRoutingComment[0].Equals(rowRouting[0]) &&
					    rowRoutingComment[1].Equals(rowRouting[1]))
					{
						row[28] = "yes";                                      // rocmmts
						hasComment = true;
						...
						for (int i = 0; i < maxPages; i++)
						{
							if ((rowRoutingComment.ItemArray.GetLength(0) > (4 + (i*15))) && !rowRoutingComment[4 + (i*15)].ToString().Equals("") && hasComment)
							{
								row[29 + (i*20)] = i + 1;                     // cd_seq
								row[30 + (i*20)] = rowRoutingComment[0];      // cd_ref
								row[31 + (i*20)] = "OP";                      // cd_type
								row[32 + (i*20)] = "FR";                      // cd_lang
								row[33 + (i*20)] = i + 1;                     // cd_seq_2
								
								for (int j = 0; j < 15; j++)
								{
									row[34 + (i*20) + j] = GetValue(rowRoutingComment[4 + j + (i*15)], "", 75);    // cd_cmmt
								}
							}
							else
							{
								row[29 + (i*20)] = "0";                               // cd_seq
								row[30 + (i*20)] = "";                                // cd_ref
								row[31 + (i*20)] = "";                                // cd_type
								row[32 + (i*20)] = "";                                // cd_lang
								row[33 + (i*20)] = "0";                               // cd_seq_2
								
								for (int j = 0; j < 15; j++)
								{
									row[34 + (i*20) + j] = "";                        // cd_cmmt
								}
				...
				foreach (DataRow rowItem in item.Rows)
				{
					if (rowItem[0].Equals(rowRouting[0]) || rowItem[78].Equals(rowRouting[0]))
					{
						if (!rowItem[18].ToString().Equals("MT"))
						{
							ok = false;
						}
			...
			foreach (DataRow rowRouting in routingV9.Rows)
			{
				...
				row[25] = "";                                // ro_wipmtl_part
				row[26] = rowRouting[26];                    // ro_mv_nxt_op
				row[27] = rowRouting[27];                    // ro_auto_lbr
				
				row[28] = rowRouting[28];                    // rocmmts

				if (row[28].ToString().ToLower().Equals("yes"))
				{
					hasComment = true;
					...
					{
						if (hasComment && 
							(!rowRouting[33 + (i*15)].ToString().Equals("") ||
						    !rowRouting[34 + (i*15)].ToString().Equals("") ||
						    !rowRouting[35 + (i*15)].ToString().Equals("") ||
						    !rowRouting[36 + (i*15)].ToString().Equals("") ||
						    !rowRouting[37 + (i*15)].ToString().Equals("") ||
						    !rowRouting[38 + (i*15)].ToString().Equals("") ||
						    !rowRouting[39 + (i*15)].ToString().Equals("") ||
						    !rowRouting[40 + (i*15)].ToString().Equals("") ||
						    !rowRouting[41 + (i*15)].ToString().Equals("") ||
						    !rowRouting[42 + (i*15)].ToString().Equals("") ||
						    !rowRouting[43 + (i*15)].ToString().Equals("") ||
						    !rowRouting[44 + (i*15)].ToString().Equals("") ||
						    !rowRouting[45 + (i*15)].ToString().Equals("") ||
						    !rowRouting[46 + (i*15)].ToString().Equals("") ||
						    !rowRouting[47 + (i*15)].ToString().Equals("")))
						{
							row[29 + (i*20)] = i + 1;                     // cd_seq
							row[30 + (i*20)] = rowRouting[0];             // cd_ref
							row[31 + (i*20)] = "OP";                      // cd_type
							row[32 + (i*20)] = "FR";                      // cd_lang
							row[33 + (i*20)] = i + 1;                     // cd_seq_2
							...
							for (int j = 0; j < 15; j++)
							{
								row[34 + (i*20) + j] = GetValue(rowRouting[33 + j + (i*15)], "", 75);    // cd_cmmt
							}
						}
						else
						{
				...
				foreach (DataRow rowItem in item.Rows)
				{
					if (rowItem[0].Equals(rowRouting[0]) || rowItem[78].Equals(rowRouting[0]))
					{
						if (!rowItem[18].ToString().Equals("TR") && !rowItem[18].ToString().Equals("LM"))
						{
							ok = false;
						}
		...
		public static System.Data.DataTable Write131_CodeProdStruct(System.Data.DataTable codeProdStruct, System.Data.DataTable codeProdStructV9, System.Data.DataTable item)
		{
			...
			foreach (DataRow rowCodeProdStruct in codeProdStruct.Rows)
			{
				...
				row[0] = GetValue(rowCodeProdStruct[0], 18);         // bom_parent
				row[1] = GetValue(rowCodeProdStruct[1], 24);         // bomdesc
				...
				foreach (DataRow rowItem in item.Rows)
				{
					if (rowItem[0].Equals(rowCodeProdStruct[0]))
					{
						ok = false;
						break;
					}
		...
		/// <returns></returns>
		public static System.Data.DataTable Write135_ProdStruct(System.Data.DataTable prodStruct, System.Data.DataTable prodStructV9, System.Data.DataTable item)
		{
			...
			foreach (DataRow rowProdStruct in prodStruct.Rows)
			{
				...
				foreach (DataRow rowItem in item.Rows)
				{
					if (rowItem[0].Equals(rowProdStruct[0]))
					{
						if (!rowItem[18].ToString().Equals("MT"))
						{
							ok = false;
		...
		public static System.Data.DataTable Write182211_ProductionLine(System.Data.DataTable productionLine)
		{
			...
			productionLineTable.Columns.Add("lnd_rate", Type.GetType("System.Int32"));
		...
		public static System.Data.DataTable Write113_Measure(System.Data.DataTable measure, System.Data.DataTable measureV9)
		{
			System.Data.DataTable measureTable = new System.Data.DataTable();
			DataRow row = null;
			
			#region Format du fichier des unites de mesure
			// Unites de mesure
			measureTable.Columns.Add("um_um", Type.GetType("System.String"));
			measureTable.Columns.Add("um_alt_um", Type.GetType("System.String"));
			measureTable.Columns.Add("um_part", Type.GetType("System.String"));
			measureTable.Columns.Add("um_conv", Type.GetType("System.Double"));
			#endregion
			
			#region Chargement des unites de mesure
			foreach (DataRow rowMeasure in measure.Rows)
			{
				row = measureTable.NewRow();
				
				#region Données unites de mesure
				row[0] = rowMeasure[0];                      // um_um
				row[1] = rowMeasure[1];                      // um_alt_um
				row[2] = rowMeasure[2];                      // um_part
				row[3] = rowMeasure[3];                      // um_conv
				#endregion
			...
			#region Chargement des unites de mesure V9
			foreach (DataRow rowMeasureV9 in measureV9.Rows)
			{
				row = measureTable.NewRow();
				
				#region Données unites de mesure
				row[0] = rowMeasureV9[0];                      // um_um
				row[1] = rowMeasureV9[1];                      // um_alt_um
				row[2] = rowMeasureV9[2];                      // um_part
				row[3] = rowMeasureV9[3];                      // um_conv
				#endregion
			...
			Build.WriteCsvFromDataTable(measureTable, "C:\\TEMP\\1.13_items.csv");
			return measureTable;
		}
		...
		private static void AddGeneral(string strField, object strValue, object strDescription, ref System.Data.DataTable generalTable)
		{
			...
			if (strValue != System.DBNull.Value)
			{
				...
				if (!exists)
				{
					...
					if (!strValue.ToString().ToLower().Equals("no") && 
					    !strValue.ToString().ToLower().Equals("n"))
					{
		...
		private static void AddRow(System.Data.DataRow row, ref System.Data.DataTable table, int keyIndex)
		{
			bool exists = false;
			...
			if (row != null)
			{
				...
				{
					if (rowTable[keyIndex].Equals(row[keyIndex]))
					{
						exists = true;
						break;
					}
				...
				if (!exists && !row[keyIndex].ToString().Equals(""))
				{
					table.Rows.Add(row);
				}
			}
